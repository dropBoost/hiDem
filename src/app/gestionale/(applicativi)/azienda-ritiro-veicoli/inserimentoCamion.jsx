...
export default function InserimentoCamion({onDisplay, setStatusAziende}) {
  ...
  const targaPresente = veicoliAzienda?.filter(t => t.targa_camion == formData?.targaCamion)
  ...
  async function handleSubmit(e) {
    ...
    } else if (targaPresente.length > 0) {
      alert("Targa già inserita")
      setFormData({...formData , targaCamion:""})
    }
    else {
  ...
  async function DeleteTarga (idCamion) {

    if (!idCamion) return

    const conferma = window.confirm("Sei sicuro di voler cancellare?");
    if (conferma) {
      const { error } = await supabase
      .from("camion_ritiro")
      .delete()
      .eq("uuid_camion_ritiro", idCamion)

      if (error) {
        console.error("Errore cancellazione:", error)
      } else {
        setUpdateCamion(prev => !prev)
        alert("Riga cancellata con successo")
      }
                ...
                {veicoliAzienda.map(v => (
                  ...
                  <div className="flex flex-row items-center justify-between rounded-xl basis-1/2 lg:basis-1/3 xl:basis-1/6 p-1">
                    <div className="flex flex-row items-center justify-between gap-2 px-3 py-2 rounded-xl h-full w-full border">
                      <TargaDesign targa={v.targa_camion} buttonFuction={()=>DeleteTarga(v.uuid_camion_ritiro)}/>
                    </div>
                  </div>      
                  </>
                ))}
